...
@interface MLNLineLayerTests : MLNStyleLayerTests
@end

@implementation MLNLineLayerTests
...
- (void)testPredicates {
    ...
    MLNLineStyleLayer *layer = [[MLNLineStyleLayer alloc] initWithIdentifier:@"layerID" source:source];
...
- (void)testProperties {
    ...
    // line-cap
    {
        ...
        XCTAssertThrowsSpecificNamed(layer.lineCap = functionExpression, NSException, NSInvalidArgumentException, @"MLNLineLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    ...
    // line-miter-limit
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.lineMiterLimit = functionExpression, NSException, NSInvalidArgumentException, @"MLNLineLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"mgl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"mgl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.lineMiterLimit = functionExpression, NSException, NSInvalidArgumentException, @"MLNLineLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // line-round-limit
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.lineRoundLimit = functionExpression, NSException, NSInvalidArgumentException, @"MLNLineLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"mgl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"mgl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.lineRoundLimit = functionExpression, NSException, NSInvalidArgumentException, @"MLNLineLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // line-blur
    {
        ...
        MLNTransition lineBlurTransition = layer.lineBlurTransition;
    ...
    // line-color
    {
        ...
        MLNTransition lineColorTransition = layer.lineColorTransition;
    ...
    // line-dasharray
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.lineDashPattern = functionExpression, NSException, NSInvalidArgumentException, @"MLNLineLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"mgl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"mgl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.lineDashPattern = functionExpression, NSException, NSInvalidArgumentException, @"MLNLineLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // line-gap-width
    {
        ...
        MLNTransition lineGapWidthTransition = layer.lineGapWidthTransition;
    ...
    // line-offset
    {
        ...
        MLNTransition lineOffsetTransition = layer.lineOffsetTransition;
    ...
    // line-opacity
    {
        ...
        MLNTransition lineOpacityTransition = layer.lineOpacityTransition;
    ...
    // line-pattern
    {
        ...
        constantExpression = [NSExpression expressionWithFormat:@"MLN_FUNCTION('image', 'Line Pattern')"];
        XCTExpectFailure(@"Awaiting unit test refactoring https://github.com/mapmetrics/mapmetrics-native/issues/421");
        NSExpression *functionExpression = [NSExpression expressionWithFormat:@"mgl_step:from:stops:($zoomLevel, %@, %@)", constantExpression, @{@18: constantExpression}];
        ...
        MLNTransition linePatternTransition = layer.linePatternTransition;
    ...
    // line-translate
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.lineTranslation = functionExpression, NSException, NSInvalidArgumentException, @"MLNLineLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"mgl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"mgl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.lineTranslation = functionExpression, NSException, NSInvalidArgumentException, @"MLNLineLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // line-translate-anchor
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.lineTranslationAnchor = functionExpression, NSException, NSInvalidArgumentException, @"MLNLineLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"mgl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"mgl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.lineTranslationAnchor = functionExpression, NSException, NSInvalidArgumentException, @"MLNLineLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // line-width
    {
        ...
        MLNTransition lineWidthTransition = layer.lineWidthTransition;
...
- (void)testValueAdditions {
    XCTAssertEqual([NSValue valueWithMLNLineCap:MLNLineCapButt].MLNLineCapValue, MLNLineCapButt);
    XCTAssertEqual([NSValue valueWithMLNLineCap:MLNLineCapRound].MLNLineCapValue, MLNLineCapRound);
    XCTAssertEqual([NSValue valueWithMLNLineCap:MLNLineCapSquare].MLNLineCapValue, MLNLineCapSquare);
    XCTAssertEqual([NSValue valueWithMLNLineJoin:MLNLineJoinBevel].MLNLineJoinValue, MLNLineJoinBevel);
    XCTAssertEqual([NSValue valueWithMLNLineJoin:MLNLineJoinRound].MLNLineJoinValue, MLNLineJoinRound);
    XCTAssertEqual([NSValue valueWithMLNLineJoin:MLNLineJoinMiter].MLNLineJoinValue, MLNLineJoinMiter);
    XCTAssertEqual([NSValue valueWithMLNLineTranslationAnchor:MLNLineTranslationAnchorMap].MLNLineTranslationAnchorValue, MLNLineTranslationAnchorMap);
    XCTAssertEqual([NSValue valueWithMLNLineTranslationAnchor:MLNLineTranslationAnchorViewport].MLNLineTranslationAnchorValue, MLNLineTranslationAnchorViewport);
}
