...
@interface MLNHeatmapLayerTests : MLNStyleLayerTests
@end

@implementation MLNHeatmapLayerTests
...
- (void)testProperties {
    ...
    // heatmap-intensity
    {
        ...
        XCTAssertThrowsSpecificNamed(layer.heatmapIntensity = functionExpression, NSException, NSInvalidArgumentException, @"MLNHeatmapLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        ...
        MLNTransition heatmapIntensityTransition = layer.heatmapIntensityTransition;
    ...
    // heatmap-opacity
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.heatmapOpacity = functionExpression, NSException, NSInvalidArgumentException, @"MLNHeatmapLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"mgl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"mgl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.heatmapOpacity = functionExpression, NSException, NSInvalidArgumentException, @"MLNHeatmapLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        // Transition property test
        ...
        MLNTransition heatmapOpacityTransition = layer.heatmapOpacityTransition;
    ...
    // heatmap-radius
    {
        ...
        MLNTransition heatmapRadiusTransition = layer.heatmapRadiusTransition;
